...
//  UnsupportedHandler onUnsupported;
  ...
  Future<String> snapshot() async {
    ...
    final screenshotData = await readFileAsBytes(path);

    await saveFile(screenshotData, 'MyAppScreenshot${DateTime.now().millisecondsSinceEpoch}.png');
  ...
  Future<void> processScreenshot() async {
    ...
    final screenshotPath = await snapshot();

    // Read the screenshot data as bytes
    final screenshotData = await readFileAsBytes(screenshotPath);

    // Save the screenshot data to the external storage
    await saveFile(screenshotData, 'MyAppScreenshot.png');
  }
